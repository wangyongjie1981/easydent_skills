...
4. 复制弹窗中的 Key（明文只显示一次），妥善保存。该 Key 具备查询、快速建档、创建/调整预约、医生排班写入、病历草稿和回访处理能力。
...
日报专用 Key 具备预约、收费、病历、回访等只读查询能力，但不具备快速建档、预约调整和医生排班写入能力。一个 MCP 连接通常只配置一个 Key；需要同时使用业务写入和完整日报时，请按当前任务切换对应 Key。MCP 不执行收费、退款、库存管理、收费单打印或患者来源维护。
...
4. 创建患者、调整预约、写入排班、保存病历草稿或处理回访时，确认 WorkBuddy 会先复述信息，并在你确认后再写入系统。
